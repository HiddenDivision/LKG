...
bot.on('raw', event =>{
	...
	if(eventname === 'MESSAGE_REACTION_ADD')
	{
		...
		if(event.d.message_id === '613462452209844273')
		{
...
bot.on('messageReactionAdd', (messageReaction, user) =>{
	...
	var verify = messageReaction.message.guild.roles.find("id", "613460774278660106");
	var def = messageReaction.message.guild.roles.find("id", "613453647476555806");
	console.log(roleName)
	var member = messageReaction.message.guild.members.find(member => member.id === user.id);
	if(member)
	{
	if(roleName === 'LKG'){
            if(messageReaction.message.channel.id === "613462182973407259"){
		member.removeRole(verify.id)
		member.addRole(def.id)
                console.log("Success.")
            }
	}
    }
...
bot.on('guildMemberAdd', member =>{
    const channel = member.guild.channels.find(channel => channel.id === '613451028175650918')
    if(!channel) return;
    let role = member.guild.roles.find("id", "613460774278660106");
    member.addRole(role.id);
    if(member.avatarURL === null){
        const embed9 = new RichEmbed()
        .setAuthor(`Thanks for joining LowkeyGeeks!`, ``, '')
        .setDescription(`Enjoy your flippin' stay here, ${member}!`)
        .setImage(`https://media.discordapp.net/attachments/514446545374937098/556273640287502396/get_geeked.gif`)
        .setColor(0x99d6ec)
	...
	switch(args[0]){
		case 'LK':
            ...
            if(args[1] === 'help'){
                ...
                .setTitle('Lowkey bot')
                .addField('General commands:', "'LK <action> <tag> <message (Optional)>'\nDescription: Use this command to have fun and interact with others!\nActions:\nslap\nkiss\nfuck\nfu\nkill\nhug\nshoot\nkms\nboop\nsuck\nlick\npat\nsuicide bomb\ncuddle\ntickle\npeep\nass rape\nkidnap\nrape\nlewd\nfbi open up\n")
                .addField('Current normal prefix', "'LK' : It's the current prefix.")
                .setColor(0x160033)
		...
		case '-announcement':
			...
			const achannel = bot.channels.find(channel => channel.id === "613451033187844155");
